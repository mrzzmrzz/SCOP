...
def load_ckpt(path, model, prefix):
    cur_state_dict = torch.load(path, map_location=torch.device("cpu"))["model"]
    update_state_dict = {}
    for key in cur_state_dict.keys():
        if key.startswith(prefix):
            update_key = key.replace(prefix + ".", "")
            update_state_dict[update_key] = cur_state_dict[key]
    model.load_state_dict(update_state_dict)
    print("{} -> state_dict load successfully!".format(path))
    return model
