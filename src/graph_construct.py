...
from torchdrug import core, data, transforms
...
class GraphConstruction(nn.Module, core.Configurable):
    ...
    def __init__(self, node_layers=None, edge_layers=None, edge_feature="residue_type", residue_feature="onehot"):
    ...
    def edge_gearnet(self, graph, edge_list, num_relation):
        ...
        if self.residue_feature == "index":
            return torch.cat([
                in_residue_type.unsqueeze(-1),
                out_residue_type.unsqueeze(-1),
                r.unsqueeze(-1),
                sequential_dist.clamp(max=self.max_seq_dist).unsqueeze(-1),
                spatial_dist.unsqueeze(-1)], dim=-1,
            )
        else:
            return torch.cat([
                functional.one_hot(in_residue_type, len(data.Protein.residue2id)),
    ...
    def forward(self, graph):
        ...
        """
        if self.residue_feature == "index":
            graph.residue_feature = torch.argmax(graph.residue_feature, dim=1).unsqueeze(-1)
...
class SpatialLineGraph(nn.Module, core.Configurable):
    ...
    def forward(self, graph):
        ...
        return type(line_graph)(edge_list, num_nodes=line_graph.num_nodes, offsets=line_graph._offsets,
                                num_edges=line_graph.num_edges, num_relation=self.num_angle_bin, edge_feature=angle.unsqueeze(-1),
